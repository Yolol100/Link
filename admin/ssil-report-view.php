...
function ssil_render_report_view($stats) {
    ...
    <div class="yoast-card yoast-p-4">
        ...
        <!-- Algemene statistieken (altijd tonen) -->
        <table class="yoast-table widefat yoast-report-table yoast-mb-4" id="ssil-report-table">
            ...
            <tbody>
                ...
                <tr><td>Meest gelinkte URL</td><td><span title="<?php echo esc_attr($stats['most_linked_url']); ?>" class="yoast-text-muted"><?php echo esc_html($stats['most_linked_url']); ?></span></td></tr>
        ...
        if (!empty($stats['post_link_counts'])) {
            ...
            <table class="yoast-table widefat yoast-report-table yoast-mb-4">
                ...
                <tbody>
                    ...
                    foreach ($stats['post_link_counts'] as $post_id => $count) {
                        if ($count == 0) continue;
                        if (++$top > 10) break;
                        echo '<tr><td><a href="' . esc_url(get_edit_post_link($post_id)) . '" target="_blank" rel="noopener" class="yoast-link">' . esc_html(get_the_title($post_id)) . '</a></td><td>' . intval($count) . '</td></tr>';
                    }
                    ?>
                </tbody>
            </table>
                            ...
                            <td>
                                ...
                                if ($posts) {
                                    ...
                                    foreach ($posts as $post_id) {
                                        ...
                                        echo '<a href="' . esc_url(get_edit_post_link($post_id)) . '" target="_blank" rel="noopener" class="yoast-link">' . esc_html(get_the_title($post_id)) . '</a><br>';
                                    }
                                } else {
                                    echo '-';
                                }
                ...
                <?php if ($show_dead): ?>
                    <li>
                        ...
                        foreach ($stats['dead_links'] as $url) {
                            echo '<span class="yoast-badge yoast-badge-error yoast-mr-2">' . esc_html($url) . '</span>';
                        }
                        ?>
                    </li>
                <?php endif; ?>
